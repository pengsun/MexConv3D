function conv3d_vs_matconvnet_bprop_gpu()
...
Y = mex_conv3d(X,F,B,...
  ...
  'stride', stride3d);
szY = size(Y);
dzdy = ran * gpuArray.randn(szY, 'single');
...
assert( all( gather(diff_dX) < tau ) );

diff_dF = abs(dF1(:)-dF2(:));
fprintf('assert dF1 - dF2 is very small\n');
assert( all( gather(diff_dF) < tau ) );

diff_dB = abs(dB1(:)-dB2(:));
fprintf('assert dB1 - dB2 is very small\n\n\n');
assert( all( gather(diff_dB) < tau ) );